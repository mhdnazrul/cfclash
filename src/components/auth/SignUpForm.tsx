...
import { Mail, Lock, Eye, EyeOff, Code, Zap } from "lucide-react";
...
export function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password || !cfHandle) { toast.error("Please fill in all fields"); return; }
    if (password.length < 6) { toast.error("Password must be at least 6 characters"); return; }

    setLoading(true);
    const valid = await validateCfHandle(cfHandle);
    if (!valid) { toast.error("Invalid Codeforces handle. Please check and try again."); setLoading(false); return; }
      ...
      <div className="max-w-md mx-auto">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[hsl(187_92%_69%/0.1)] border border-[hsl(187_92%_69%/0.2)] mb-6">
            ...
            <span className="text-sm text-[hsl(187,92%,69%)] font-medium">Join the Arena</span>
          </div>
          <h1 className="text-3xl font-bold text-foreground mb-2">Create Account</h1>
          <p className="text-muted-foreground">Start competing in real-time battles</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">
                Codeforces Handle <span className="text-destructive">*</span>
              </label>
              <div className="relative">
                <Code className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                <input type="text" value={cfHandle} onChange={(e) => setCfHandle(e.target.value)} placeholder="Your Codeforces username"
        ...
        <p className="mt-6 text-center text-muted-foreground">
          ...
          <Link to="/auth/signin" className="text-primary hover:text-primary/80 transition-colors">Sign in</Link>
        </p>
      </div>
    </section>
  );
